...
import { editCourier } from "@/actions/courier";
import { InputField } from "@/components/inputField";

export default function EditCourierForm({ user }) {
  const initialState = {
    success: false,
    message: "",
  };

  const [state, action, isPending] = useActionState(editCourier, initialState);
    ...
    <form action={action}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <input type="hidden" name="oldPhone" value={user.phone} />
        </div>
      </div>
    </form>
  );
